...
class FoodController extends Controller
{
    ...
    {
        $data = [
            'product' => Food::with('restaurant','food_review')
                ->where('slug', $slug)
                ->firstOrFail(),
            'reviews' => FoodReview::with('product')
                ->where('status', 1)
        ...
        $data['og']['og_title'] = $data['product']->name;
        $data['og']['og_description'] = $data['product']->description;
        $data['og']['og_image'] = $data['product']->feature_photo;
        $data['og']['og_image_src'] = asset('uploads/product/feature/' . $data['product']->feature_photo);
        return view('front.product.single_food')->with($data);
    }
        ...
        if ($request->isMethod('post')) {
            ...
            if ($data = $this->commentAbility($commentData['food_id'])) {
                ...
                if (Carbon::now() < $databaseNextDay) {
                    ...
                    return redirect('product/' . $restaurant->slug . '/' . $food->slug . '/' . '#review-message');
                }
            }
