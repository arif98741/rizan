    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">Food Reviews</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Food Reviews</h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                @foreach($reviews as $key=> $review)
                                    <tr>
                                        ...
                                        <td>{{ $review->ip }}</td>
                                        <td>{{ date('d/m/Y',strtotime($review->created_at)) }}
                                        <br>
                                            {{ date('h:i:sa',strtotime($review->created_at)) }}
                                        </td>

                                        <td>

                                            @if($review->status == 0)
                                                <a href="{{ url('admin/food_review/change-status/'.$review->id.'/1') }}"
                                                   class="btn btn-sm btn-warning">Pending</a>
                                            @else
                                                <a href="{{ url('admin/food_review/change-status/'.$review->id.'/0') }}"
                                                   class="btn btn-sm btn-success">Published</a>
                                            @endif

                                            <a class="btn btn-warning btn-sm"
                                               href="{{ route('admin.food_review.destroy',$review->id) }}"
                                            ...
                                            <form id="vendor-delete-form{{ $review->id }}"
                                                  ...
                                                  action="{{ route('admin.food_review.destroy',$review->id) }}"
