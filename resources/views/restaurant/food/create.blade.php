...
@section('content')
    <main class="appContent-res-adm" id="appcontnt">
        <form action="{{ route('restaurant.product.store') }}" method="post" enctype="multipart/form-data">
            @method('post')
            @csrf
            <div class="form-group row">
                <label for="itemName" class="col-sm-2 col-form-label">Enter food name :</label>
                <div class="col-sm-9">
                    <input type="text" name="name" class="form-control" value="{{ old('name') }}" placeholder="food name...">
                    @error('name')
